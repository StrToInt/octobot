...
class OctobotCommands:
    ...
    def __init__(self, octobot, bot, dispatcher, settings: OctobotSettings):
        ...
        self.__bot = bot
        self.__settings = OctobotSettings(settings)
        self.__dispatcher = dispatcher
        self.__command_cb = CallbackData('id','action')  # post:<id>:<action>
    ...
    def get_main_keyboard(self):
        return types.InlineKeyboardMarkup().row(
            types.InlineKeyboardButton('❔ Статус', callback_data=self.__command_cb.new(action='kb_status')),
            types.InlineKeyboardButton('📸Фото', callback_data=self.__command_cb.new(action='kb_photo')),
            types.InlineKeyboardButton('🖨Печать...', callback_data=self.__command_cb.new(action='kb_print')),
        ).add(types.InlineKeyboardButton('📛STOP', callback_data=self.__command_cb.new(action='kb_stop_request'))).row(
            types.InlineKeyboardButton('� Настройки', callback_data=self.__command_cb.new(action='kb_show_settings')),
            types.InlineKeyboardButton(self.__octobot.get_smile_for_boolean(self.__settings.is_silent())+' Silent', callback_data=self.__command_cb.new(action='kb_silent_toggle')),
            types.InlineKeyboardButton('📲Действия', callback_data=self.__command_cb.new(action='kb_show_actions')),
        )
